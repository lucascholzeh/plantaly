...
**Ao acrescentar frase:** confira a atribuição numa fonte antes de escrever, e preencha `fonte` com a URL — mesma disciplina do catálogo. O cabeçalho de `src/frases/frases.ts` registra três atribuições que já foram **reprovadas** na conferência; não as reintroduza. **Onda 2 (2026-09-24):** as 9 frases originais foram aposentadas a pedido do Lucas e trocadas por 50, todas conferidas na parte com fonte do Wikiquote (nunca em "Disputed" ou "Misattributed"). `frases.test.ts` impede as aposentadas de voltar. Cuidado especial com a frase famosa que "todo mundo sabe de quem é": "quem tem um porquê enfrenta qualquer como" é de Nietzsche, não de Frankl, e a mais citada de Marco Aurélio na internet não existe nas Meditações.
...
**Como regar (2026-09-24).** `src/catalogo/comoRegar.ts` diz o gesto e a quantidade de cada espécie; aparece na ficha da planta, na da espécie e numa linha da aba "Hoje". **Quantidade nunca em ml:** o volume depende do vaso, e tamanho de vaso está fora de escopo (design, seção 3). As fontes também não dão ml — dão critério no vaso ("até escorrer", "até a superfície umedecer"). Quando a fonte da espécie não fala em volume, `quantoPelaRegraGeral: true` e a quantidade vem do guia de rega em vasos da RHS, dito como tal na tela. Espécie nova sem bloco quebra a build.
